CREATE DATABASE conservation_db2
...
-- manual data id already 3, so do work.
SELECT setval(pg_get_serial_sequence('rangers', 'ranger_id'), 3, true);
...
-- Problem 7: Update species discovered before year 1800 to 'Historic'
UPDATE species
SET conservation_status = 'Historic'
WHERE discovery_date < '1800-01-01';

-- Problem 8: Label sighting’s time of day
SELECT sighting_id,
    CASE
...
-- Problem 9: Delete rangers who never sighted any species
DELETE FROM rangers
WHERE ranger_id NOT IN (
    SELECT DISTINCT ranger_id FROM sightings
);

--Note 9 number problem google and Another Source FOr Better In This code.
-- DELETE FROM rangers
-- WHERE NOT EXISTS (
--     SELECT 1
--     FROM sightings
